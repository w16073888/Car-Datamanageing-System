...
void QuotePage::setupUI()
{
    ...
    connect(m_orderCompleter, &SearchCompleter::selected, this, [this](int idx) {
        if (idx >= 0 && idx < m_orderRows.size()) {
            {   // 回填工单号时抑制 textChanged，避免触发二次搜索/二次下拉
...
void QuotePage::onOrderSearch()
{
    ...
    // 无论 1 条还是多条，都在输入框下方自动展开下拉，仅在用户主动选择时回填加载
    QList<QVariant> ids;
    for (int i = 0; i < m_orderRows.size(); ++i)
        ids << i;                     // 用行号作为 id，选中后按 m_orderRows 回填
...
void QuotePage::loadOrderInfo(const QString &orderNo)
{
    ...
    for (int i = 0; i < partItems.size(); i++) {
        ...
        priceItem->setData(Qt::UserRole + 1, it.price);   // 记录原始单价，保存时据此定位明细行
...
// ============================================================
void QuotePage::savePartPriceEdits()
{
    if (m_currentOrderId == 0) return;
    ...
    for (int r = 0; r < m_partsTable->rowCount(); r++) {
        for (int colBase : {0, 7}) {
            ...
            // 与库房工作台-确认提单(saveBillingPriceEdits)保持一致，
            // 保证出库报表(t_inventory_log)也能读到改后价
...
void QuotePage::onSettle()
{
    ...
    if (QMessageBox::question(this, "确认结算",
            QString("确认结算此工单？\n\n工单号: %1\n应收合计: ¥%2\n\n"
                    "所有费用将在本次结算一次性付清，结算后状态将变为「已结算」，不可撤销。")
            .arg(m_currentOrderNo)
    ...
    if (q.exec() && q.numRowsAffected() > 0) {
        // ===== 写入结算记录 =====
        {
            ...
            sq.bindValue(":by", Session::instance().isLoggedIn()
                              ...
                              : QVariant());
            if (!sq.exec()) {
                qWarning() << "[QuotePage] 写入结算记录失败:" << sq.lastError().text();
            }
        }
...
// ============================================================
QString QuotePage::buildSettlementHtmlFor(int orderId, bool hideCost)
{
    ...
    // 材料表头与合计行（按 hideCost 决定是否含"成本/总成本"列）
    QString partsHeader = hideCost
        ? "<tr><th>序号</th><th>配件件号</th><th>配件名称</th><th>单位</th><th>数量</th><th>单价</th><th>金额</th></tr>"
        : "<tr><th>序号</th><th>配件件号</th><th>配件名称</th><th>单位</th><th>数量</th><th>成本</th><th>总成本</th><th>单价</th><th>金额</th></tr>";
    QString partsSubtotal = hideCost
        ? "<tr class='subtotal'><td colspan='6' align='right'>材料费合计</td>"
    ...
    html.replace("%LABORROWS%",   laborRowsHtml);
    html.replace("%LABORTOTAL%",  fmt(displayLabor));
    html.replace("%PARTSHEADER%", partsHeader);
    html.replace("%PARTSROWS%",   partsRowsHtml);
    html.replace("%PARTSSUBTOTAL%", partsSubtotal);
    html.replace("%PARTSTOTAL%",  fmt(partsTotal));
    html.replace("%MFEE%",        fmt(partsTotal));
    html.replace("%MGM%",         fmt(mgmtFee));
    html.replace("%LFEE%",        fmt(displayLabor));
