...
void CustomerVisitPage::onLoadSettledOrders()
{
    ...
    QString sql = QString(
        "SELECT w.id, w.order_no AS '工单号', v.owner_name AS '车主', v.owner_phone AS '车主电话', "
        "  v.plate_number AS '车牌号', w.total_amount AS '金额', s.settled_at AS '结算时间' "
        "FROM t_workorder w "
...
void CustomerVisitPage::onSelectOrder(const QModelIndex &index)
{
    ...
    m_lblOrderInfo->setText(QString("工单号：%1 | 车牌：%2 | 金额：%3")
                           .arg(orderNo,
                                m_model->data(m_model->index(index.row(), 4)).toString(),
                                m_model->data(m_model->index(index.row(), 5)).toString()));
}
...
void CustomerVisitPage::onSaveVisit()
{
    ...
    // 满意度与备注写入 t_workorder，同时将回访状态置为已回访（单条 UPDATE，走通用 execute）
    ...
    if (query.exec() && query.numRowsAffected() > 0) {
        ...
        QMessageBox::warning(this, "失败", query.lastError().text());
    }
}
